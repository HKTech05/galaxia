...
export default function StaycationSidebar({ basePath, propertyName, overrideNav }: { basePath: string, propertyName: string, overrideNav?: { name: string, href: string }[] }) {
                ...
                <div className="h-20 flex items-center px-8 border-b border-slate-100">
                    <div className="flex items-center gap-3">
                        <div>
                            ...
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Staycation • {propertyName}</p>
                        </div>
                    </div>
                </div>
                ...
                <nav className="flex-1 px-4 py-8 overflow-y-auto space-y-2">
                    {overrideNav ? (
                        overrideNav.map((nav) => {
                            ...
                            return (
                                <Link
                                    ...
                                    href={nav.href}
                                    onClick={() => setIsMobileOpen(false)}
                                    className={`flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-200 group ${isActive ? 'bg-emerald-50 text-emerald-700 shadow-sm shadow-emerald-100' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}
                                >
                                    <LayoutDashboard
                                        size={20}
                                        className={isActive ? "text-emerald-600" : "text-slate-400 group-hover:text-slate-600 transition-colors"}
                                    />
                                    <span className={`font-semibold text-[15px] ${isActive ? "" : "font-medium"}`}>
                                        {nav.name}
                                    </span>
                        ...
                        <Link
                            href={basePath}
                            onClick={() => setIsMobileOpen(false)}
                            className={`flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-200 group bg-emerald-50 text-emerald-700 shadow-sm shadow-emerald-100`}
                        >
                            <LayoutDashboard
                                size={20}
