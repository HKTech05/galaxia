...
import { Plus, X, Upload, IndianRupee, Clock, Users, Calendar as CalendarIcon, MoreVertical, CreditCard, Ticket, CheckCircle2, ChevronRight, ChevronLeft, CalendarDays, Search, Camera, ArrowLeft, Ban, User, FileText, UserPlus } from "lucide-react";
...
export default function Admin1Dashboard() {
    ...
    const [newExtraGuest, setNewExtraGuest] = useState({ name: '', idType: 'Aadhar Card', idFileName: '', amount: '1500', paymentMode: 'UPI' });

    const handleAddExtraGuest = () => {
        if (!newExtraGuest.name || (!newExtraGuest.idFileName && newExtraGuest.idType !== "NA")) return;
        setExtraGuests([...extraGuests, { ...newExtraGuest, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }]);
        setNewExtraGuest({ name: '', idType: 'Aadhar Card', idFileName: '', amount: '1500', paymentMode: 'UPI' });
        setShowExtraGuestForm(false);
    };

    const handleExtraGuestIdUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files.length > 0) {
            setNewExtraGuest({ ...newExtraGuest, idFileName: e.target.files[0].name });
        }
    };
                    ...
                    <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8 bg-slate-50">
                        ...
                        <div className="mt-8 border-t border-slate-200 pt-6">
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2"><Users size={16} /> Extra Guests</h3>
                                {!showExtraGuestForm && (
                                    <button onClick={() => setShowExtraGuestForm(true)} className="text-xs font-bold text-emerald-600 hover:text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-md flex items-center gap-1">
                                        ...
                                        <div key={idx} className="bg-slate-50 border border-slate-200 rounded-lg p-3 flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs font-medium text-slate-500">Added at {guest.time} · {guest.idType}</p>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-sm font-bold text-emerald-600">+₹{guest.amount}</p>
                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Paid via {guest.paymentMode}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            {extraGuests.length === 0 && !showExtraGuestForm && (
                                <div className="text-center py-6 bg-slate-50 border border-slate-200 rounded-lg">
                                    <p className="text-xs font-medium text-slate-500">No extra guests added post check-in.</p>
                                </div>
                            )}

                            {/* Add Extra Guest Form */}
                            {showExtraGuestForm && (
                                <div className="bg-white border-2 border-emerald-100 rounded-xl p-4 shadow-sm animate-in fade-in slide-in-from-top-2">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                        <div className="space-y-1">
                                            <label className="text-xs font-bold text-slate-700 uppercase">Guest Name</label>
                                            <input
                                                type="text"
                                                value={newExtraGuest.name}
                                                onChange={e => setNewExtraGuest({ ...newExtraGuest, name: e.target.value })}
                                        ...
                                        <div className="space-y-1">
                                            <label className="text-xs font-bold text-slate-700 uppercase">ID File Document</label>
                                            <div className="flex gap-2">
                                                <select
                                                    value={newExtraGuest.idType}
                                                    onChange={e => setNewExtraGuest({ ...newExtraGuest, idType: e.target.value })}
                                                    className="w-1/3 bg-slate-50 border border-slate-200 rounded-lg px-2 py-2 text-xs focus:ring-2 focus:ring-emerald-500/20 outline-none"
                                                >
                                                ...
                                                <div className="relative w-2/3">
                                                    <label className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm focus-within:ring-2 focus-within:ring-emerald-500/20 outline-none flex items-center justify-between cursor-pointer overflow-hidden">
                                                        <span className={`truncate text-xs ${newExtraGuest.idFileName ? 'text-slate-700 font-bold' : 'text-slate-400'}`}>
                                                            {newExtraGuest.idFileName || "Upload ID Document"}
                                                        </span>
                                                        <Upload size={14} className="text-slate-400 shrink-0 ml-2" />
                                                        <input
                                                            type="file"
                                                            accept="image/*,.pdf,.doc,.docx"
                                                            onChange={handleExtraGuestIdUpload}
                                                            className="hidden"
                                                        />
                                                    </label>
                                                </div>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                                        <div className="space-y-1">
                                            <label className="text-xs font-bold text-slate-700 uppercase">Surcharge Amount</label>
                                            <div className="relative">
                                                <IndianRupee size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                                                <input
                                                    type="number"
                                                    value={newExtraGuest.amount}
                                                    onChange={e => setNewExtraGuest({ ...newExtraGuest, amount: e.target.value })}
                                                    className="w-full bg-slate-50 border border-slate-200 rounded-lg pl-8 pr-3 py-2 text-sm font-bold focus:ring-2 focus:ring-emerald-500/20 outline-none text-emerald-700"
                                                />
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            ...
                                            <select
                                                value={newExtraGuest.paymentMode}
                                                onChange={e => setNewExtraGuest({ ...newExtraGuest, paymentMode: e.target.value })}
                                                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold text-emerald-700 focus:ring-2 focus:ring-emerald-500/20 outline-none"
                                            >
                                                <option>UPI</option>
                                                <option>Cash</option>
                                                <option>Card</option>
                                    ...
                                    <div className="flex items-center gap-3 pt-4 border-t border-emerald-100">
                                        <button onClick={() => setShowExtraGuestForm(false)} className="flex-1 py-2 text-sm font-bold text-slate-500 hover:text-slate-700 transition-colors">Cancel</button>
                                        <button onClick={handleAddExtraGuest} className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white py-2 rounded-lg text-sm font-bold shadow-sm shadow-emerald-600/20 transition-colors">
                                            Save Guest & Payment
                                        </button>
                                    </div>
                                </div>
