...
class DynamicDataService {
  ...
  async checkStaycationAvailability(propertySlug, checkIn, checkOut, subPropertySlug = null) {
    try {
      ...
      if (!subPropertySlug && (property.slug === "ambrose" || property.slug === "amstel-nest")) {
        ...
        for (const sub of allSubs.rows) {
          const bRes = await pool.query(
            `SELECT id FROM staycation_bookings
             WHERE property_id = $1 AND sub_property_id = $2
               AND status NOT IN ('cancelled', 'no_show', 'transferred')
               AND (check_in_date + INTERVAL '5 hours 30 minutes')::date < $4::date
               AND (check_out_date + INTERVAL '5 hours 30 minutes')::date > $3::date`,
            [property.id, sub.id, cleanCheckIn, cleanCheckOut]
          );
          const blkRes = await pool.query(
            `SELECT id FROM blocked_dates
             WHERE property_id = $1 AND sub_property_id = $2
               AND (blocked_date + INTERVAL '5 hours 30 minutes')::date >= $3::date
               AND (blocked_date + INTERVAL '5 hours 30 minutes')::date < $4::date`,
            [property.id, sub.id, cleanCheckIn, cleanCheckOut]
          );
          const bookedOrBlocked = bRes.rows.length + blkRes.rows.length;
          const cap = sub.unit_count || 1;
          const isAvail = bookedOrBlocked < cap;
          subResults.push({
            subPropertyName: sub.name,
            subPropertySlug: sub.slug,
            capacity: cap,
            bookedCount: bRes.rows.length,
            blockedCount: blkRes.rows.length,
            availableUnits: Math.max(0, cap - bookedOrBlocked),
            isAvailable: isAvail,
            statusText: isAvail ? `${cap - bookedOrBlocked} of ${cap} available` : "SOLD OUT / BOOKED"
          });
        }
      ...
      // Query bookings that overlap
      let bookingQuery = `
        SELECT id, booking_ref, check_in_date, check_out_date, sub_property_id
      ...
      `;
      let params = [property.id, cleanCheckIn, cleanCheckOut];
      if (subProperty) {
        bookingQuery += " AND sub_property_id = $4";
        params.push(subProperty.id);
      }
      const bookingsRes = await pool.query(bookingQuery, params);

      // Query blocked dates
      let blockQuery = `
        SELECT blocked_date, sub_property_id
        FROM blocked_dates
        WHERE property_id = $1
          AND (blocked_date + INTERVAL '5 hours 30 minutes')::date >= $2::date
          AND (blocked_date + INTERVAL '5 hours 30 minutes')::date < $3::date
      `;
      let blockParams = [property.id, cleanCheckIn, cleanCheckOut];
      if (subProperty) {
        blockQuery += " AND sub_property_id = $4";
        blockParams.push(subProperty.id);
      }
      const blockedRes = await pool.query(blockQuery, blockParams);
      ...
      const activeBookingsCount = bookingsRes.rows.length;
      const blockedCount = blockedRes.rows.length;
      const totalBookedOrBlocked = activeBookingsCount + blockedCount;
      const isAvailable = totalBookedOrBlocked < capacity;
      console.log("[DynamicDataService] checkStaycationAvailability RESULT:", { cleanCheckIn, cleanCheckOut, capacity, bookedCount: activeBookingsCount, blockedCount, isAvailable });
      ...
      return {
        ...
        blockedCount,
        availableUnits: Math.max(0, capacity - totalBookedOrBlocked),
        isAvailable,
        statusText: isAvailable ? `${capacity - totalBookedOrBlocked} unit(s) available` : "NOT AVAILABLE / FULLY BOOKED"
      };
    } catch (err) {
