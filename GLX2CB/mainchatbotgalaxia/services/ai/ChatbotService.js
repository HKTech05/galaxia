...
Output ONLY a raw valid JSON object (no markdown, no backticks, no other text) with the following structure:
{
  ...
  async processMessage(sessionId, text, customerPhone, phoneNumberId, botType, platform = "whatsapp") {
    ...
    // Check Phone Call Request for Amstel Nest (Bot 2) & Staycation (Bot 3)
    if (cleanType === "amstel_nest" || cleanType === "amstel" || cleanType === "bot2") {
      ...
      if (callRegex.test(textTrimmed)) {
        const callMsg = "For verbal inquiries or booking assistance for Amstel Nest Karjat, you can call us directly on: +91 99877 34458 (tel:+919987734458).";
        await conversationService.saveUserMessage(sessionId, textTrimmed, customerPhone, phoneNumberId, botType, platform);
      ...
      // Check Bot 2 Generalized Price Inquiry (price, rates, no just say price)
      const isGeneralPriceQuery = /^(price|prices|rate|rates|cost|tariff|no\s*just\s*say\s*price|tell\s*me\s*price|give\s*me\s*price|price\s*list|rate\s*list|price\s*kya\s*hai|kitna\s*hai|price\s*tell|just\s*price)$/i.test(textTrimmed);
      if (isGeneralPriceQuery) {
        const generalPriceMsg = "Amstel Nest Standard Cottage (per night, 2 adults + meals):\n- Mon-Thu: *₹4,950*\n- Fri/Sun: *₹5,950*\n- Saturday: *₹6,950*\n- 2 Oct (Prime Date): *₹6,950*\n- 3 Oct (Prime Date): *₹7,950*\n\nFamily Cottage (4 adults + meals):\n- Mon-Thu: *₹9,000*\n- Fri/Sun: *₹10,000*\n- Saturday: *₹12,000*\n- 2 Oct (Prime Date): *₹11,000*\n- 3 Oct (Prime Date): *₹13,000*\n\nExtra adult: *₹2,000*, Kids 5–12 yrs: *₹1,000*, under 5 free. All prices excl. 5% GST.\n\nSecurity deposit is *₹2,000* (refundable), payable at check-in via Cash or UPI. It is not included in the stay price.";
        await conversationService.saveUserMessage(sessionId, textTrimmed, customerPhone, phoneNumberId, botType, platform);
        const savedMsg = await conversationService.saveAssistantMessage(cleanSessionId, generalPriceMsg);
    ...
    } else if (cleanType === "staycation" || cleanType === "bot3") {
      const callRegex = /\b(call|phone|calling|contact\s*number|phone\s*number|call\s*number|call\s*kare|call\s*karu|call\s*par|talk\s*on\s*call|speak\s*on\s*call)\b/i;
      if (callRegex.test(textTrimmed)) {
        const callMsg = "For verbal inquiries or booking assistance for Galaxia Karjat Staycation Villas, you can call us directly on: +91 81695 19564 (tel:+918169519564).";
        await conversationService.saveUserMessage(sessionId, textTrimmed, customerPhone, phoneNumberId, botType, platform);
        const savedMsg = await conversationService.saveAssistantMessage(cleanSessionId, callMsg);
        return { reply: callMsg, latency: Date.now() - startTime, tokenUsage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 }, cost: 0, retrievedChunksCount: 0, cached: false, messageId: savedMsg.id };
      }
    ...
    const bookingQueryRegex = /\b(my\s*booking|existing\s*booking|booking\s*done|booking\s*details|booking\s*screenshot|booking\s*confirmed|booking\s*query|booking\s*doubt)\b/i;
    if (bookingQueryRegex.test(textTrimmed)) {
      const bookingPhoneNum = cleanType === "amstel_nest" || cleanType === "amstel" || cleanType === "bot2" ? "+91 99877 34458 (tel:+919987734458)" : "+91 81695 19564 (tel:+918169519564)";
      const queryMsg = `If you have an existing booking and have questions about it, please reply with your:\n1. *Booking Name*\n2. *Booking ID*\n3. *Phone Number*\n\nI will verify your reservation details for you. Please note that for any booking modifications or date changes, you will need to call our booking team at ${bookingPhoneNum}.`;
      await conversationService.saveUserMessage(sessionId, textTrimmed, customerPhone, phoneNumberId, botType, platform);
      const savedMsg = await conversationService.saveAssistantMessage(cleanSessionId, queryMsg);
      return { reply: queryMsg, latency: Date.now() - startTime, tokenUsage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 }, cost: 0, retrievedChunksCount: 0, cached: false, messageId: savedMsg.id };
    }
    ...
    if (isDateQuery || dateIndicators.some(ind => textLowerCheck.includes(ind))) {
      ...
      if (intent) {
        ...
        if (effectiveCheckIn) {
          console.log(`[ChatbotService] Performing Staycation availability check across ALL properties for ${effectiveCheckIn} to ${effectiveCheckOut}`);
          const { pool } = require("../db");
          const propsRes = await pool.query("SELECT id, name, slug FROM properties WHERE slug != 'digital-diaries' AND is_active = true ORDER BY id ASC");
          const allPropsAvail = [];
          for (const p of propsRes.rows) {
            const subSlug = (p.slug === effectivePropSlug ? effectiveSubPropSlug : null);
            ...
            allPropsAvail.push(avail);
          }
          dynamicContext += `### REAL-TIME LIVE DATABASE CALENDAR AVAILABILITY FOR ALL PROPERTIES (AUTHORITATIVE):\n${JSON.stringify(allPropsAvail, null, 2)}\n*CRITICAL INSTRUCTION FOR AI*: You MUST use the above live database availability for ${effectiveCheckIn} to answer availability accurately for the requested property AND any alternative properties. If a property or villa is marked isAvailable: false or "SOLD OUT / BOOKED", it is strictly FULLY SOLD OUT for that date. NEVER suggest or list any property/villa as an available alternative if it is marked as SOLD OUT / FULLY BOOKED in the live database output above! If ALL properties are sold out for that date, state clearly that all Galaxia Karjat properties are completely sold out for that date.\n\n`;
        }
    ...
    await conversationService.saveUserMessage(sessionId, textTrimmed, customerPhone, phoneNumberId, botType, platform);
    const savedMsg = await conversationService.saveAssistantMessage(cleanSessionId, cleanReplyText);
    ...
    if (!isDateQuery && configManager.get("CACHE_ENABLED") && textTrimmed.length > 5 && textTrimmed.length < 50) {
      this.cache[cacheKey] = cleanReplyText;
      this.saveCache();
    }
