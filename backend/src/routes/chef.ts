...
router.post("/submit", async (req: AuthRequest, res) => {
    try {
        ...
        const waMessage = `*Galaxia Resorts — Kitchen Requirements*\n\nDaily ingredients checklist for *${dateStr}* is ready.\n\n📄 *Download Checklist PDF:*\n${downloadLink}\n\nGenerated by: ${req.admin!.username}`;

        const recipientPhone = "9653176436";
        const waSuccess = await sendWhatsAppMessage("stay1", recipientPhone, waMessage);
